...
-- 일별 매출 조회
SELECT o.OrderDate AS Date, SUM(od.Quantity * p.Price) AS Sales
FROM OrderDetails od JOIN Products p ON od.ProductID = p.ProductID
					 JOIN Orders o ON od.OrderID = o.OrderID
GROUP BY o.OrderDate;

SELECT * FROM Employees;
-- 직원별 처리금액 직원, 오더, 오더디테일, 물건
SELECT e.lastName AS Name, SUM(p.Price*od.Quantity) AS 금액
...
ORDER BY 판매금액 DESC;
-- 고객별 소비금액
SELECT c.CustomerID, c.CustomerName, SUM(p.Price * od.Quantity) 소비금액
FROM 
	Orders o JOIN OrderDetails od ON o.OrderID = od.OrderID
